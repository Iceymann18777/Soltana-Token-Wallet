...
export function useConnectionConfig(): {endpoint: error;
setEndpoint: error;
}  {
  let context = useContext(ConnectionContext);
  if (!context) {
...
export function useSolanaExplorerUrlSuffix(): string | string | string  {
...
export function refreshAccountInfo(connection, publicKey, clearCache = false): void  {
  const cacheKey = tuple(connection, publicKey.toBase58());
  refreshCache(cacheKey, clearCache);
}

export function setInitialAccountInfo(connection, publicKey, accountInfo): void  {
  const cacheKey = tuple(connection, publicKey.toBase58());
